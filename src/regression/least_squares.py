...
def least_squares_algorithm(input_data, num_folds=10):
    ...
    for train_index, test_index in kf.split(X):
        ...
        E_matrix_square = calculate_square_error_matrix(
            Y_train_array, prediction_array)
        E_matrix_absolute = calculate_absolute_error_matrix(
            Y_train_array, prediction_array)

        average_square_error_training = np.sum(
            E_matrix_square)/len(E_matrix_square)
        average_absolute_error_training = np.sum(
        ...
        E_matrix_square = calculate_square_error_matrix(
            Y_test_array, prediction_array_test)
        E_matrix_absolute = calculate_absolute_error_matrix(
            Y_test_array, prediction_array_test)
        average_square_error_testing = np.sum(
            E_matrix_square)/len(E_matrix_square)
